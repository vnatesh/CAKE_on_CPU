...
void pack_A_csr_to_sp_k_first(csr_t* csr, float* A_p, int M, int K, int nz, int p, 
   ...
   float* vals = csr->vals;
   int* colind = csr->colind;
   int* rowptr_csr = csr->rowptr;

   
   int m, k, m_cb, k_c_t, p_used, core;

   char* nnz_outer = (char*) calloc(nz , sizeof(char)); // storing number of nonzeros 
                                                                     // in each outer prod col of A

   int* k_inds = (int*) calloc(nz , sizeof(int)); // storing kc_ind 
                                                                     // of each outer prod col of A

   char* loc_m = (char*) calloc(nz , sizeof(char)); // array for storing M dim C writeback location for each nnz in A
                               // each value ranges from 0 to mr-1


   int* nnz_tiles = (int*) calloc((M_padded / m_r)*Kb + 1 , sizeof(int)); 
   int* num_col_tile = (int*) calloc((M_padded / m_r)*Kb + 1, sizeof(int)); 
   nnz_tiles[0] = 0;
   num_col_tile[0] = 0;
      ...
      for(m = 0; m < Mb; m++) {
         ...
         for(k = 0; k < Kb; k++) {
           ...
           for(core = 0; core < p_used; core++) {
            ...
            val_ind = rowptr_csr[m*p*m_c + core*m_c_x];

            csr_to_ob_A_sp(&vals[val_ind], 
               &colind[val_ind], 
               &rowptr_csr[m*p*m_c + core*m_c_x],
               &nnz_tiles[ntiles + 1],
               &num_col_tile[ntiles + 1],
               &nnz_outer[nz_col_curr], 
               &k_inds[nz_col_curr], 
               &loc_m[nz_curr],
               &A_p[nz_curr], M, m*p*m_c, core*m_c_x,
   ...
   sp_pack->nnz_tiles = nnz_tiles;
   sp_pack->num_col_tile = num_col_tile;
