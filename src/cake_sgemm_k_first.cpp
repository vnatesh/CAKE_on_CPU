...
double cake_sgemm_k_first(float* A, float* B, float* C, int M, int N, int K, int p, 
	...
	if(packedA) {
		...
		A_sz = cake_sgemm_packed_A_size(M, K, p, x, cake_cntx, KMN);
	...
	if(beta != 0) {
		clock_gettime(CLOCK_REALTIME, &start);
	    C_sz = cake_sgemm_packed_C_size(M, N, p, x, cake_cntx, KMN);
	    ...
	    C_sz = cake_sgemm_packed_C_size(M, N, p, x, cake_cntx, KMN) / sizeof(float);
	    C_p = (float*) calloc(C_sz, sizeof(float));
	}
