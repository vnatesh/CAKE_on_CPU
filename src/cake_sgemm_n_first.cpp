...
printf("n first\n");
	...
	init_block_dims(M, N, K, p, x, cake_cntx, NKM);
	...
	if(packedA) {
		...
		A_sz = cake_sgemm_packed_A_size(M, K, 1, x, cake_cntx, NKM);
	 ...
	 //    A_p = (float*) calloc(A_sz, sizeof(float));
		pack_A_single_buf_n_first(A, A_p, M, K, p, x, cake_cntx);
	...
	if(beta != 0) {
		clock_gettime(CLOCK_REALTIME, &start);
	    C_sz = cake_sgemm_packed_C_size(M, N, 1, x, cake_cntx, NKM);
