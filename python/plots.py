...
def mem_req_barplot():
	...
	ind = np.arange(N) 
	width = 0.35       
	plt.bar(ind, cake, width, label='Cake')
	plt.bar(ind + width, mkl, width,label='MKL')
	plt.ylabel("Time (% of total clockticks)")
	plt.title('(a) Percent of Time Stalled On Memory Requests')
	...
	width = 0.35       
	plt.bar(ind, cake, width, label='Cake')
	plt.bar(ind + width, armpl, width,label='ARMPL')
	plt.ylabel("Number of Requests")
	plt.title('(b) Cache and DRAM Access In ARM')
	plt.xticks(ind + width / 2, tuple(labels))
	plt.legend(loc='best')
