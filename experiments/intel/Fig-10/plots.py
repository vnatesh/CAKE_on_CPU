...
def plot_cake_vs_mkl_cpu(M,N,K,mc,kc,alpha,fname = 'cake_vs_blis', ntrials=10):
	...
	for i in range(len(NUM_CPUs)):
		for j in range(1,ntrials+1):
			...
			#
			df1 = pandas.read_csv('reports/report_blis_%d-%d.csv' % (NUM_CPUs[i],j),skiprows=17,skipfooter=16)
			df2 = pandas.read_csv('reports/report_blis_%d-%d.csv' % (NUM_CPUs[i],j),skipfooter=20)
